...
export function ShowcaseSection({ vehicle, locale, priority, align = "center", image, className }: Props) {
  ...
  return (
    <section className={cn("snap-section relative flex min-h-[100svh] flex-col overflow-hidden bg-carbon text-white", className)}>
      <Image
      ...
      <div className={cn("relative flex flex-1 flex-col justify-between px-5 pb-10 pt-28 sm:px-8 sm:pb-14 lg:px-12", align === "center" ? "items-center text-center" : "items-start")}>
        <Reveal className={cn("max-w-3xl", align === "center" && "flex flex-col items-center")}>
          <p className={cn("mb-3 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-white/70")}>
            <span className={cn("size-1.5 rounded-full", vehicle.brand === "xiaomi" ? "bg-mi" : "bg-tesla")} />
            {vehicle.brand === "xiaomi" ? t.nav.xiaomi : t.nav.tesla}
            {isInventory ? <span className="ml-2 rounded-pill bg-white/15 px-2 py-0.5 text-[10px]">{t.common.inventoryOnly}</span> : null}
            {isOverseas ? <span className="ml-2 rounded-pill bg-white/15 px-2 py-0.5 text-[10px]">{t.common.overseasOnly}</span> : null}
          </p>
          <h2 className="text-balance text-4xl font-semibold tracking-tight sm:text-5xl lg:text-6xl">{pick(vehicle.name, locale)}</h2>
          <p className="mt-3 text-pretty text-base text-white/80 sm:text-lg">{pick(vehicle.tagline, locale)}</p>
        </Reveal>
            ...
            {stats.map((s) => (
              <div key={s.label.zh} className={cn(align === "center" && "text-center")}>
                <div className="flex items-baseline gap-1 text-2xl font-semibold tabular-nums lg:text-3xl">
                  {s.value}
                  {s.unit ? <span className="text-sm font-medium text-white/70">{s.unit}</span> : null}
                </div>
                <div className="mt-0.5 text-xs text-white/60">{pick(s.label, locale)}</div>
              </div>
            ))}
          </div>
          <div className={cn("flex w-full max-w-md flex-col gap-3 sm:w-auto sm:max-w-none sm:flex-row", align === "center" && "sm:justify-center")}>
            <Button href={isOverseas ? `/vehicles/${vehicle.slug}` : `/vehicles/${vehicle.slug}/design`} variant="light" size="lg" className="w-full sm:w-64">
              {isOverseas ? t.nav.learnMore : t.nav.design}
            </Button>
            <Button href={isOverseas ? `/vehicles/${vehicle.slug}#interest` : `/test-drive?vehicle=${vehicle.slug}`} variant="glass" size="lg" className="w-full sm:w-64">
              {isOverseas ? (locale === "zh" ? "登记关注" : "Register interest") : t.nav.testDrive}
            </Button>
          </div>
          <div className={cn("mt-4 flex items-center gap-3 text-xs text-white/70", align === "center" && "justify-center")}>
            <span>

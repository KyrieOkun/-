...
import { hashString, secureCode, seededRandom } from "./utils";
...
export interface GarageVehicle {
  ...
  state: {
    ...
    /** Set on charge_start; charge progress is derived from it, not from unrelated commands. */
...
export function computeStatus(gv: GarageVehicle, now = Date.now()): VehicleStatus {
  ...
  if (gv.state.charging) {
    const since = gv.state.chargeStartedAt ?? gv.state.lastCommandAt;
    const minutesSince = since ? Math.max(0, (now - new Date(since).getTime()) / 60000) : 0;
...
export function applyCommand(gv: GarageVehicle, command: GarageCommand, payload?: { temp?: number }): GarageVehicle {
  ...
  switch (command) {
    ...
    case "charge_start":
      state.charging = true;
      state.chargeStartedAt = state.lastCommandAt;
      state.pluggedIn = true;
      break;
    case "charge_stop":
      state.charging = false;
      state.chargeStartedAt = undefined;
      break;
    case "sentry_on":
      state.sentry = true;
      break;
...
export function generateDemoVin(brand: "xiaomi" | "tesla"): string {
  const wmi = brand === "tesla" ? "LRW" : "HXM";
  // Random, VIN-alphabet-safe (no I/O/Q); collisions are checked by the caller.
  return `${wmi}${secureCode(14).replace(/[IOQ]/g, "X")}`;
}
